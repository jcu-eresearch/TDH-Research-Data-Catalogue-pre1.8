...
//JCU: redefined to support the Affiliation combos
function showMintNlaLookupDialog(e, lookup_source) {
	var input_sibs = $(e).parent().find('input');
	var ids = new Array();
    for (i = 0; i < input_sibs.length; i++) {
	...
	//JCU: Added to find the Affiliation combos
    var input_sibs2 = $(e).parent().parent().find('select');
    for (i = 0; i < input_sibs2.length; i++) {
        if (input_sibs2[i].type == 'select-one') {
            ids.push(input_sibs2[i].id);
        }
    }
	NameLookUp(ids, lookup_source.split(','))
}
...
function NameLookUp(ids, lookup_source) {
	...
	//JCU: altered length-1 to '3' due to the added Affiliation combos
	for (var i = 2; i< 3; i++) { // skip the last hidden field: dcIdentifier
...
//	ids[0]: title; ids[1]: firstName; ids[2]: surName; ids[3]: mintId
	...
	//JCU: redefined to support the Affiliation combos
	old_btn_callback = btn_callback;
	btn_callback = function (ok, result) {
		old_btn_callback(ok, result);
		if (ok) {
			var selectedFunc = 'lookupDone';
			try {
				var nlaId = result["nlaId"];
