function loadOpenLayers() {
    ...
    function init(){
        ...
        if  (layers == "mapquest"){
            osmLayer = new OpenLayers.Layer.XYZ("OpenStreetMap", ["https://otile1-s.mqcdn.com/tiles/1.0.0/map/${z}/${x}/${y}.png",
                                                                 ...
                                                                 {attribution: "Data, imagery and map information provided by <a href='http://www.mapquest.com/'  target='_blank'>MapQuest</a>, <a href='http://www.openstreetmap.org/' target='_blank'>Open Street Map</a> and contributors, <a href='http://creativecommons.org/licenses/by-sa/2.0/' target='_blank'>CC-BY-SA</a>  <img src='https://developer.mapquest.com/content/osm/mq_logo.png' border='0'>", 
                                                                  ...
                                                                  wrapDateLine: true});
            imageryLayer = new OpenLayers.Layer.XYZ("Imagery", ["https://otile1-s.mqcdn.com/tiles/1.0.0/sat/${z}/${x}/${y}.png",
                                                                ...
                                                                {attribution: "Tiles Courtesy of <a href='http://open.mapquest.co.uk/' target='_blank'>MapQuest</a>. Portions Courtesy NASA/JPL-Caltech and U.S. Depart. of Agriculture, Farm Service Agency. <img src='https://developer.mapquest.com/content/osm/mq_logo.png' border='0'>",
                                                                 transitionEffect: "resize",
                                                                 wrapDateLine: true});
        }
        ...
        if (layers == "mapquest") {
            map.addLayers([osmLayer, imageryLayer, vLayer]);
            map.addControl(new OpenLayers.Control.LayerSwitcher());
        }
    ...
    function featureUpdate(event) {
		...
		for(var i=0; i < locationElements.size(); i++) {
    		var textField = $(locationElements[i]).find(".redboxGeoDataFid");
    		if(textField.val().trim() == fId ) {
        		found = true;
    ...
    function completeFeatureUpdate(event, fIdElement, rowElement){
        ...
        fIdElement.val(event.feature.id);
        ...
        if (type != null && type != "") {
        	...
        	function setValue(rowElement){
        		...
        		if (dropDown.length == 0){
        			setTimeout(function(){setValue(rowElement)}, 100);
        		}
        		else{
            		dropDown.val("text");
        		}
        ...
        var toolCell = rowElement.find(".jaffaDeleteItem");
        rowElement.find(".jaffaDeleteItem").find("button").hide();
        ...
        var trapDiv = toolCell.parent().find(".trapClickDiv");
        if (trapDiv.size() == 0) {
            trapDiv = $("<div class=\"trapClickDiv\"></div>");
            toolCell.before(trapDiv);
