...
class AlertsData:
    def __activate__(self, context):
        ...
        #This is used per-file to track exceptions in the mapping file
    ...
    def __processDir(self):
        ...
        for file in files:
            ...
            if file.endswith(".csv"):
                ...
                ## XML Files
                mapPath = self.config.getString(None, ["alerts", "xmlMaps", "rif"])
                (success, failed) = self.__processXML(file, mapPath)
            else:
    ...
    def __xmlToJson(self, fileName, xmlMappings, xmlExceptions, xmlDefaultNamespace):
        self.log.info("Converting '{}' to JSON...", fileName)
        filePath = self.pBase(fileName)
        timestamp = time.gmtime(os.path.getmtime(filePath))
